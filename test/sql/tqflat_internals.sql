...
-- Tests M1.2 (rotation), M1.3 (codebook), M1.4 (encode/decode round-trip)

SET enable_seqscan = on;

-- M1.3: codebook for 2 bits, dim 128: 3 boundaries + 4 centroids = 7 values
SELECT array_length(tqflat_test_codebook(128, 2), 1);

-- M1.3: middle boundary (index 2 of 3 for nLevels=4) should be ~0 (symmetric Beta)
SELECT abs((tqflat_test_codebook(128, 2))[2]) < 1e-3;

-- M1.2: rotation matrix for dim=64 should be orthogonal (max|R*R' - I| < 1e-3)
SELECT tqflat_test_rotation_orthogonality(64) < 1e-3;

-- M1.4: round-trip error at bits=2 should be greater than at bits=4 for a fixed vector
...
-- M3.1: asymmetric inner-product estimator (tqflat_test_ip_estimate).
