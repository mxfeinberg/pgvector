...
-- Cosine: use a table with distinct directions so there are no distance ties.
-- v = [g, 301-g, 0, 0] for g=1..300 gives each id a unique angle.
-- Nearest to [1,0,0,0] (cosine) are ids 300,299,298 (highest ratio of dim-1 to dim-2).
CREATE TABLE tqhnsw_cos2 (id int, v vector(4));
INSERT INTO tqhnsw_cos2 SELECT g, ARRAY[g, 301-g, 0, 0]::real[]::vector FROM generate_series(1, 300) g;
