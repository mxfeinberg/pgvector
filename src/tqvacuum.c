 ...
 *
 * Space reclamation: v1 is tombstone-only.  Deleted entries remain on their
 * pages; space is NOT reclaimed until a full REINDEX or a future compaction
 * milestone.  VACUUM FULL rebuilds the index from scratch via ambuild, which
 * does reclaim space.
 */
 ...
 *
 * If ambulkdelete was not called (analyze-only path), stats is NULL.  In that
 * case we return NULL to indicate no changes (mirrors ivfflatvacuumcleanup).
 *
 * When stats is provided, fill in num_pages and (if we have a bulk-delete
 * result already) leave num_index_tuples as set by tqbulkdelete.  We do not
 * re-scan the index here; the live count from the bulk-delete pass is
 * sufficient.
 *
