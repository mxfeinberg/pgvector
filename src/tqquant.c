 ...
 *
 * M1.2: TqBuildRotation  -- deterministic Gaussian + modified Gram-Schmidt
 * M1.3: TqBuildCodebook  -- grid-based Lloyd-Max on Beta coordinate density
 * M1.4: TqEncode + pack/unpack + scale computation
 */
 ...
 * iterate TQ_LLOYD_ITERS times alternating
...
TqEncode(const TqModel *model, const float *vec, TqEntry *entry)
{
	...
	/* 5. Scale = norm / <y, yhat>.  If inner product is tiny, fall back. */
	entry->scale = (fabs(ip) > 1e-12) ? (float) (norm / ip) : (float) norm;
	...
	if (model->tqProd)
	{
		...
		rnorm = sqrt(rnorm);
		entry->residualNorm = (float) rnorm;
