 ...
 * Populated by RemoveHeapTids; consulted by NeedsUpdated to decide which     *
 * surviving elements have edges pointing at soon-to-be-removed nodes.        *
 * ------------------------------------------------------------------------- */
 ...
 * non-entry-point element for later entry-point replacement.                 *
...
RemoveHeapTids(TqhnswVacuumState *vs)
{
	...
	/* Read the current entry point once so it is never chosen as highest. */
		...
		for (offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno))
		{
			...
			if (!ItemPointerIsValid(&etup->heaptid))
			{
				...
				DeletedAdd(vs->deleted, blkno, offno);
					 ...
					 !(blkno == entryBlkno && offno == entryOffno))
			{
				/* Track highest-level surviving non-entry point. */
...
NeedsUpdated(TqhnswVacuumState *vs, TqhnswElement *element)
{
	 ...
	 * prior insert.  Mirrors HNSW's check on the final slot.
	 */
	if (!needsUpdated)
	{
		Assert(ntup->count > 0);
...
RepairGraphEntryPoint(TqhnswVacuumState *vs)
{
	...
	if (highestPoint != NULL)
	{
		...
		if (NeedsUpdated(vs, highestPoint))
		{
			TqhnswElement *ep;

			TqhnswGetMetaInfo(index, NULL, NULL, NULL, &entryBlkno, &entryOffno,
							  &entryLevel, NULL, NULL);
			ep = LoadByTid(vs, entryBlkno, entryOffno, cache, vs->tmpCtx);
...
/*
 * Bulk delete tuples from the index (three-pass tombstone + graph repair).
 */
				 ...
				 IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	/* Pass 3: mark dead element tuples deleted and reclaim slots. */
	MarkDeleted(&vs);
